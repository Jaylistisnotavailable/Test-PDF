...
export type LegacyShape = PointShape|LineShape|PolylineShape|PolygonShape|RectShape|CircleShape|TextShape|MeasureShape;
...
const initialState:DrawingState={
  activeTool:'select', shapes:[], selectedShapeIds:[],
  currentStrokeColor:'#2563eb',currentFillColor:'transparent',
  currentStrokeWidth:2,currentFontSize:16,currentOpacity:1,
  scaleNumerator:1,scaleDenominator:100,scaleUnit:'mm',
  undoStack:[],redoStack:[],clipboard:[],historyTransaction:'idle',
};
...
export const drawingSlice=createSlice({
  ...
  reducers:{
    ...
    updateShape:(s,a:PayloadAction<{id:string;changes:Partial<Shape>}>)=>{
      ...
      if(shape) Object.assign(shape,a.payload.changes,{updatedAt:new Date().toISOString()});
    },
    ...
    selectShape:(s,a:PayloadAction<{id:string;multiSelect?:boolean}>)=>{
      ...
      } else s.selectedShapeIds=[a.payload.id];
    },
    selectShapes:(s,a:PayloadAction<string[]>)=>{s.selectedShapeIds=[...a.payload];},
    clearSelection:s=>{s.selectedShapeIds=[];},
    deleteSelected:s=>{
      const ids=new Set(s.selectedShapeIds);
      s.shapes=s.shapes.filter(x=>!ids.has(x.id));s.selectedShapeIds=[];
    },
    copySelected:(s)=>{
      s.clipboard=s.shapes.filter(x=>s.selectedShapeIds.includes(x.id)).map(x=>JSON.parse(JSON.stringify(x)));
    },
    pasteClipboard:(s)=>{
      ...
      const pasted=s.clipboard.map(x=>{
        ...
        copy.id=nanoid();copy.createdAt=now;copy.updatedAt=now;
        if('geometry' in copy){
          const prefix=copy.type==='portalFrame'?'PF':copy.type==='column'?'C':copy.type==='beam'?'B':copy.type==='wall'?'W':'S';
          const used=s.shapes.filter((v:any)=>v.type===copy.type).map((v:any)=>v.label);
          let n=1;while(used.includes(`${prefix}-${String(n).padStart(3,'0')}`))n++;
          copy.label=`${prefix}-${String(n).padStart(3,'0')}`;
          (copy as any).properties.label=copy.label;
        }
        if('geometry' in copy){
          const g=copy.geometry as any;
          if(copy.type==='column'){g.x+=200;g.y+=200;}
          else if(copy.type==='slab'){g.points=g.points.map((p:any)=>({x:p.x+200,y:p.y+200}));}
          else {g.start={x:g.start.x+200,y:g.start.y+200};g.end={x:g.end.x+200,y:g.end.y+200};}
        } else if('x' in copy){copy.x+=200;copy.y+=200;}
        else if('points' in copy){for(let i=0;i<copy.points.length;i+=2){copy.points[i]+=200;copy.points[i+1]+=200;}}
        return copy;
      });
      s.shapes.push(...pasted);s.selectedShapeIds=pasted.map(x=>x.id);
      s.clipboard=pasted.map(x=>JSON.parse(JSON.stringify(x)));
    },
    setStrokeColor:(s,a:PayloadAction<string>)=>{s.currentStrokeColor=a.payload;},
    setFillColor:(s,a:PayloadAction<string>)=>{s.currentFillColor=a.payload;},
    setStrokeWidth:(s,a:PayloadAction<number>)=>{s.currentStrokeWidth=a.payload;},
    setFontSize:(s,a:PayloadAction<number>)=>{s.currentFontSize=a.payload;},
    setOpacity:(s,a:PayloadAction<number>)=>{s.currentOpacity=a.payload;},
    setScaleRatio:(s,a:PayloadAction<{num:number;den:number;unit:string}>)=>{s.scaleNumerator=a.payload.num;s.scaleDenominator=a.payload.den;s.scaleUnit=a.payload.unit;},
    _pushUndo:(s,a:PayloadAction<Shape[]>)=>{
      s.undoStack.push(JSON.parse(JSON.stringify(a.payload)));s.redoStack=[];
      if(s.undoStack.length>50)s.undoStack.shift();
    },
    undo:s=>{
      const previous=s.undoStack.pop();
      if(previous){s.redoStack.push(JSON.parse(JSON.stringify(s.shapes)));s.shapes=previous;s.selectedShapeIds=[];}
    },
    redo:s=>{
      const next=s.redoStack.pop();
      if(next){s.undoStack.push(JSON.parse(JSON.stringify(s.shapes)));s.shapes=next;s.selectedShapeIds=[];}
    },
    importShapes:(s,a:PayloadAction<Shape[]>)=>{s.shapes.push(...a.payload);},
    _setHistoryState:(s,a:PayloadAction<'idle'|'active'>)=>{s.historyTransaction=a.payload;},
  },
});
...
export const undoableMiddleware:Middleware=storeAPI=>next=>action=>{
  ...
  if(a.type===beginHistoryTransaction.type){
    ...
    if(state.drawing.historyTransaction==='idle'){
      (storeAPI.dispatch as typeof storeAPI.dispatch)(drawingSlice.actions._setHistoryState('active'));
  ...
  if(a.type==='drawing/undo'||a.type==='drawing/redo'||a.type===beginHistoryTransaction.type||a.type===endHistoryTransaction.type||a.type==='drawing/_pushUndo'||a.type==='drawing/_setHistoryState')
    return next(a);
  if(isHistoryAction(a)){
    ...
    if(state.drawing.historyTransaction==='idle'){
      ...
      const result=next(a);
      if(JSON.stringify(before)!==JSON.stringify((storeAPI.getState() as RootState).drawing.shapes))
...
export const {
  setActiveTool,addShape,updateShape,deleteShape,selectShape,selectShapes,clearSelection,deleteSelected,
  copySelected,pasteClipboard,setStrokeColor,setFillColor,setStrokeWidth,setFontSize,setOpacity,
  setScaleRatio,_pushUndo,undo,redo,importShapes,_setHistoryState
...
export const selectShapesByPage=(s:RootState,pageIndex:number)=>s.drawing.shapes.filter(x=>x.pageIndex===pageIndex);
export const selectShapesByLayer=(s:RootState,layerId:string)=>s.drawing.shapes.filter(x=>x.layerId===layerId);
export const selectSelectedShapes=(s:RootState)=>s.drawing.shapes.filter(x=>s.drawing.selectedShapeIds.includes(x.id));
