...
export function StructuralPropertyDialog({
  ...
  const fields = useMemo(() => {
    ...
    switch (element.type) {
      case 'column': return ['label', 'width', 'depth', 'rotation', 'section', 'material'];
      case 'beam': return ['label', 'width', 'depth', 'section', 'material'];
      case 'wall': return ['label', 'thickness', 'wallType', 'material'];
      case 'slab': return ['label', 'thickness', 'level', 'material'];
      case 'portalFrame': return [
        'label', 'height', 'columnWidth', 'columnDepth', 'beamWidth', 'beamDepth', 'section', 'material',
      ];
  ...
  useEffect(() => {
    ...
    // Slab thickness is intentionally stored as an engineering mm value.
    if (element.type === 'slab') {
      data.thickness = p.thickness;
    }

    setDraft(Object.fromEntries(Object.entries(data).map(([key, value]) => [
      key,
      String(value ?? ''),
    ])));
  ...
  const apply = () => {
    ...
    for (const key of fields) {
      ...
      if (pageGeometryFields.has(key) && key in g) {
        // Column/beam/wall/portal geometry is persisted in PDF page points.
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="mb-2 text-xs text-muted-foreground">
          Geometric dimensions are shown in real engineering millimetres. Drawing scale: 1:{scaleDenominator}.
          Viewer zoom is not part of this calculation.
        </div>
