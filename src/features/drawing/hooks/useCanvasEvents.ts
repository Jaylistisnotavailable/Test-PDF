...
//   return shape.type === 'column' || shape.type === 'beam' || shape.type === 'wall' || shape.type === 'slab' || shape.type === 'portalFrame';
// }
...
//       const engineering = pagePointToEngineeringUnit(point, currentCoordinateSystem);
//       emitCursorCoordinate({
//         pageIndex: currentPageRef.current,
//         pagePoint: point,
//         engineeringPoint: engineering,
//         unit: currentCoordinateSystem.unit,
//         scaleNumerator: currentCoordinateSystem.scaleNumerator,
//         scaleDenominator: currentCoordinateSystem.scaleDenominator,
//       });
//     };
...
//       return toolInstances[currentTool] ?? toolInstances.select;
//     };
...
//       canvas.style.cursor = originModeRef.current ? 'crosshair' : tool.cursor;
//     };
//     updateCanvasCursor();
...
//       return state.drawing.shapes.filter((shape): shape is StructuralElement =>
...
//         dispatch(setPageOrigin({ pageIndex: currentPageIndex, x: point.x, y: point.y }));
...
//       if (currentActiveTool === 'select' && !currentHitTest(point.x, point.y)) {
...
//       const state = store.getState();
//       const structural = isStructuralTool(currentActiveTool);
...
//       if (currentActiveTool !== 'beam' && currentActiveTool !== 'wall' && currentActiveTool !== 'portalFrame') {
//         setSnapPointRef.current(null);
//       }
//     };
