...
export interface CanvasEvent {
  x:number;y:number;rawEvent:React.MouseEvent|MouseEvent;
}
export interface ToolContext {
 dispatch:AppDispatch;getState:()=>RootState;pdfScale:number;
 tempShape:Shape|null;setTempShape:(shape:Shape|null)=>void;
 hitTest:(x:number,y:number)=>Shape|null;showTextDialog:(x:number,y:number)=>void;
 addShape:(shape:Shape)=>void;
 updateShape:(id:string,changes:Partial<Shape>)=>void;
 selectShape:(id:string,multi:boolean)=>void;
 clearSelection:()=>void;deleteSelected:()=>void;
 beginHistory:()=>void;endHistory:()=>void;
}
